...
console.log(removeThree(2, array));
...
const arrayJoin = ['Hello', 'world', 'this', 'is', 'an', 'array'];
console.log(joinArray(arrayJoin));
...
const listFoods = (recipe) => {
  let result = [];
  let indexes = [15, 9, 9, 9, 10, 8, 8, 8, 8, 10, 10];
  for (let i = 0; i < indexes.length; i++) {
    result.push(recipe.ingredients[i].slice(indexes[i], recipe.ingredients[i].length));
  }
  return result;
};
