...
console.log(checkValues(obj, 2));
console.log(checkValues(obj, 4));
...
const updateNumbers = (obj) => {
  ...
  for (const [name, phoneNumber] of Object.entries(obj)) {
    const formattedEntry = `${name}: ${phoneNumber}`;
...
const getHouses = (arr) => {
  ...
  arr.forEach(character => {
    if (!houses.includes(character.houses)) {
...
const hasChildrenValues = (arr, characterName) => {
  const characterObj = arr.find(character => character.name === characterName);

  if (characterObj && characterObj.children && characterObj.children.length > 0) {
    return true;
  }
