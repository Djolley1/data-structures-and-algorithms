...
const screenForNames = (arr) => {
  const regex = /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s[a-zA-Z]+(\s[a-zA-Z]+)?$/;
  const screenedNames = arr.filter(name => regex.test(name));
  return screenedNames;
}
...
const screenedNames = screenForNames(names);
console.log(screenedNames);
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
let biggerThanLuke = (arr) => {
  ...
  if (!lukeCharacter) return '';

  const lukeMass = lukeCharacter.mass;

  const names = arr.filter(character => parseInt(character.mass) > parseInt(lukeMass)).map(character => character.name);
...
const detectTicTacToeWin = (board) => {
  for (let i = 0; i < 3; i++) {
    if (helpCheck(board, i, 0, i, 1, i, 2)) return true;
  }

  for (let i = 0; i < 3; i++) {
    if (helpCheck(board, 0, i, 1, i, 2, i)) return true;
  }

  if (helpCheck(board, 0, 0, 1, 1, 2, 2)) return true;
  if (helpCheck(board, 0, 2, 1, 1, 2, 0)) return true;
